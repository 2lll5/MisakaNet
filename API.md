...
> **Version:** 2.16.0 | **Protocol:** `misaka-protocol.json`
...
### 4.2 Master API (`hub/master/master_api.py`)

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/status` | GET | Node health and sync status |
| `/nodes` | GET | Registered node directory |
| `/sync/trigger` | POST | Force immediate sync |

### 4.3 Federation HMAC Auth (`hub/federation/hmac_auth.py`)
...
verified = verify_request(signed, secret_key)
```

### 4.4 Knowledge Graph (`hub/storage/knowledge_graph.py`)
...
kg.query("concept")  # Semantic query
```
...
Optional notification channels for sync events.

### 5.1 Discord / Slack / Email (`hub/sync/notifier.py`)
...
### 5.2 Feishu (Lark) (`hub/sync/feishu_notifier.py`)
